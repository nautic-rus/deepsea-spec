...
class HTTPManager extends Actor {
  ...
  val routes: Route = {
    concat(
      ...
      (get & path("hullNestingByProjectAndBlocks") & parameter("project") & parameter("blocks")) { (project, blocks) =>
        askFor(ActorManager.spec, GetHullNestingByProjectAndBlocks(project, blocks))
      },
    )
  }
