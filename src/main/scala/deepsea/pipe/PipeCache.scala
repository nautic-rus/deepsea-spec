...
class PipeCache extends Actor{
  ...
  override def preStart(): Unit = {
    ...
    system.scheduler.scheduleWithFixedDelay(600.seconds, 20.minutes, self, UpdatePipeJoints())
  }
            ...
            while (rs.next()) {
              ...
              pipeSegs += PipeSegExtended(project = proj, zone = rs.getString("ZONENAME") match {
                ...
                typeCode = rs.getString("TYPECODE") match {
                  ...
                  case value: Double => value
                  case _ => 0
                }, zone_id = rs.getInt("ZONE") match {
                  case value: Int => value
                  case _ => 0
                }, system_id = rs.getInt("SYSTEM") match {
                  case value: Int => value
                  case _ => 0
                }, nomD = rs.getDouble("ND1MM") match {
                  case value: Double => value
                  case _ => 0
                }, outD = rs.getDouble("OUTDIAMETER") match {
                  case value: Double => value
                  case _ => 0
                },
                List.empty[UsrPar],
