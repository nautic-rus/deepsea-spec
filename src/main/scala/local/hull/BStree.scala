...
trait BStree  {
  ...
  def initHullPartList(project: String, taskId: String, docNum: String, docName: String, user: String): String = {
    val mongo = mongoClient()
    val collectionMat: MongoCollection[HullPL] =mongo.getDatabase(App.conf.getString("mongo.db")).withCodecRegistry(codecRegistry).getCollection("partlists")
    val sp: Option[HullPL] = {
      try {
        val existSP: HullPL = Await.result(collectionMat.find[HullPL](new BasicDBObject("name", docNum)).first().toFuture(), Duration(4, SECONDS))
        if (existSP != null) {
          Option[HullPL](existSP)
        } else {
          None
  ...
  def getHullPartList(docNum: String): String = {
    val mongo = mongoClient()
    val collectionMat: MongoCollection[HullPL] = mongo.getDatabase(App.conf.getString("mongo.db")).withCodecRegistry(codecRegistry).getCollection("partlists")
    try {
      val existSP: HullPL = Await.result(collectionMat.find[HullPL](new BasicDBObject("name", docNum)).first().toFuture(), Duration(4, SECONDS))
