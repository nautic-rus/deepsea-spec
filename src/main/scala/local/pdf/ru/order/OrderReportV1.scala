  ...
  private class PartListBodyPage(docNameEN: DocNameEN, currPage: Int) {
    ...
    def proceedOrderListHeader(debug: Boolean = true): Table = {
      ...
      def processStaticText(): Unit = {
        ...
        setStampText(cellBuff(4), "СКЛАД", italic = false, bold = true)
        setStampText(cellBuff(5), "ФАКТ", italic = false, bold = true)

        setStampText(cellBuff(6), "ВЕС(kg)", italic = false, bold = true)
        setStampText(cellBuff(7), "ВЕС ОБЩ(kg)", italic = false, bold = true)
