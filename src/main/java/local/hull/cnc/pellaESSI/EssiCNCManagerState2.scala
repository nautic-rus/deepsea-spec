...
object EssiCNCManagerState2 {
  ...
  private def doContours(inList: List[PseudoMachineOps]): List[String] = {
    ...
    inList.foreach(contour => {
      ...
      val pOffset: Point = offsetPoint(pOffsetOrig._1, pOffsetOrig._2, freeMovePoint, isOuter)
  ...
  private def offsetPoint(src: Point, beforeP: Point, nextP: Point, isOuter: Boolean): Point = {
    ...
    if(1.0-cosPHI<0.01){
      ...
      if (retVal.x.isNaN || retVal.y.isNaN) Point(bpcp(0), bpcp(1)) else retVal
    }
