...
object EssiCNCManager2 {
  ...
  private case class Arc(sp: Point, rotCenter: Point, ep: Point) {
    ...
    val radius=Math.sqrt((rotCenter.x - sp.x) * (rotCenter.x - sp.x) + (rotCenter.y - sp.y) * (rotCenter.y - sp.y))

    val radius2=Math.sqrt((rotCenter.x - ep.x) * (rotCenter.x - ep.x) + (rotCenter.y - ep.y) * (rotCenter.y - ep.y))

    val maxLen=Math.sqrt(radius*radius+radius*radius)

    def R: String = (radius*10).toInt.toString


    def isArc: Boolean = {
      val dx: Double = sp.x - ep.x
      val dy = sp.y - ep.y
      val n=norm( DenseVector[Double](dx,dy))

      //if (Math.abs(dx) + Math.abs(dy) > 0.1) true else false

      if (n > 28.0 && radius<9999.9 && radius2<9999.9) true else false
