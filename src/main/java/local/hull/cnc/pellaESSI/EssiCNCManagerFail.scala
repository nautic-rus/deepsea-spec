...
object EssiCNCManagerFail {

  private var offset: Double = 0.01
  ...
  private def doContours(inList: List[PseudoMachineOps]): List[String] = {
    ...
    inList.foreach(contour => {
      ...
      val pOffset: Point = cutOffseCalc(freeMovePoint, freeMovePointNextPoint, isOuter)

            val dx = freeMovePoint.x - currPos.x
            val dy = freeMovePoint.y - currPos.y
...
/*      val pOffset: Point = cutOffseCalc(freeMovePoint, freeMovePointNextPoint, isOuter)
      ...
      val startOffsetPoint:Point=firstOffseCalc(currPos,freeMovePointNextPoint,isOuter)
      contourPathPoints+=startOffsetPoint
      val dxS = startOffsetPoint.x - currPos.x
      val dyS = startOffsetPoint.y - currPos.y
      buff += Point(dxS, dyS).toESSI
      currPos = startOffsetPoint
      ...
      contour.ops.foreach(op => {
        ...
        op.pointOrArc match {
          case Right(arcOrig: Arc) => {
            ...
            if ((Math.abs(dx) + Math.abs(dy)) / 2 - offset > 1) {
              buff += Point(dx, dy).toESSI
              currPos = arc.sp
          ...
          case Left(pOrig: Point) => {
            val p: Point = {
              if (currentIndex <= lastOpIndex - 1) {
                ...
                pOrig.doOffset(currPosOrig, nextPoint, isOuter)
              } else {
                // pOrig
  ...
  */
  ...
  private def firstOffseCalc(sp: Point, ep: Point, isOuter: Boolean = false): Point = {
    /*    (x, y) rotated 90 degrees around(0, 0) is (-y, x). If you want to rotate clockwise , you simply do it the other way around, getting(y, -x)*/
    ...
    val offsetted: DenseVector[Double] = p + (offset * offsetDir)
