SELECT
    ...
    (SELECT DESCRIPTION FROM PRD_GEO_STD WHERE NAME IN (SELECT NORM FROM AS_STD_PART_PLATE WHERE OID = MOD_OID AND AS_OID = AS_OID) AND ROWNUM = 1) AS NORM_DESCR,
    (SELECT STORAGE_CODE FROM STD_PLATE WHERE MATERIAL_OID IN (SELECT OID FROM MATERIAL WHERE THICKNESS = PLATEMATCONTTHICKNESS AND CODE = PLATEMATCONT AND ROWNUM = 1) AND ROWNUM = 1) AS PLATE_STOCK_CONT
FROM(
        SELECT
            MOD_OID,
            AS_OID,
            ...
            (SELECT MATQ FROM AS_PART_CONT_PLATE WHERE OID = MOD_OID) AS PLATEMATCONT,
            (SELECT ROUND(THICKNESS) FROM AS_PART_CONT_PLATE WHERE OID = MOD_OID) AS PLATEMATCONTTHICKNESS
        FROM(
                SELECT
                    OID AS MOD_OID,
                    AS_OID,
